...
export default async function FeaturedProductsSection({ section }: { section: SectionInstance }) {
  const limit = num(section.data, "limit", 8);
  const products = await getFeaturedProducts(limit);
  if (products.length === 0) return null;
  const title = str(section.data, "title", "Featured Products");
